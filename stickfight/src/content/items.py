...
from systems.weapons import WEAPON_CACHE

# Convert WeaponDefinitions to Inventory Items
GENERATED_WEAPONS = []
for w in WEAPON_CACHE:
    # Average damage for the simplistic item class
    avg_dmg = (w.min_dmg + w.max_dmg) // 2
    value = avg_dmg * 10
    desc = f"{w.rarity} {w.name}. Dmg: {w.min_dmg}-{w.max_dmg} Spd: {w.speed}"
    item = Weapon(w.name, desc, avg_dmg, w.speed, w.range, value)
    GENERATED_WEAPONS.append(item)

# Hand-crafted Unique Items
...
ALL_ITEMS = UNIQUE_ITEMS + GENERATED_WEAPONS
